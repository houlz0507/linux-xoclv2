...
static struct device_node *of_ep_alloc_node(struct pci_dev *pdev, const char *name)
{
	...
	full_name_len = strlen(name) + sizeof(u64) * 2 + 3;
	full_name = devm_kmalloc(&pdev->dev, full_name_len, GFP_KERNEL);
	if (!full_name)
		return NULL;

	node->parent = of_root;
	snprintf(full_name, full_name_len, "/%s@%llx", name,
