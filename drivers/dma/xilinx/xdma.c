 ...
 * @dir: Transfering direction of the channel
 * @cfg: Transfering config of the channel
 ...
 * @dir: Transfering direction of the request
	...
	for (i = 0; i < pdata->max_dma_channels; i++) {
		...
		if (ret) {
			dev_err(&xdev->pdev->dev,
				"failed to read channnel id: %d", ret);
			return ret;
		}
...
static int xdma_xfer_start(struct xdma_chan *xdma_chan)
{
	...
	/* link all descriptors for transfering */
...
static irqreturn_t xdma_user_isr(int irq, void *dev_id)
{
	...
	if (user_irq->handler)
		user_irq->handler(irq, user_irq->arg);
...
static irqreturn_t xdma_channel_isr(int irq, void *dev_id)
{
	...
	/*
	 * if all data blocks are transfered, remove and complete the
	 * request
	 */
	if (!req->sgl) {
		list_del(&vd->node);
...
static void xdma_irq_fini(struct xdma_device *xdev)
{
	...
	if (ret) {
		dev_err(&xdev->pdev->dev, "failed to disable channel intr: %d",
			ret);
	}
