					     ...
					     const char *src_dtb, char *dst_dtb)
{
	...
	rc = xrt_md_get_prop(DEV(xdev), src_dtb, NULL, NULL, XRT_MD_PROP_LOGIC_UUID, &val, &sz);
	if (rc) {
				     ...
				     const char *dtb, const char *epname, const char *regmap)
{
	...
	rc = xrt_md_get_prop(DEV(xdev), dtb, epname, regmap,
				      ...
				      u64 msgid, bool sw_ch, u64 offset, u64 size)
{
	...
	dtb = xmgmt_mailbox_user_dtb(xdev);
	if (!dtb)
		return;
				  ...
				  enum XCLBIN_CLOCK_TYPE type, u64 *freq, u64 *freq_cnter)
{
	...
	clkname = xrt_clock_type2epname(type);
	if (!clkname)
		clkname = "UNKNOWN";
	clkxdev = xleaf_get_leaf_by_epname(xdev, clkname);
	if (!clkxdev) {
...
static int xmgmt_mailbox_get_icap_idcode(struct xmgmt_mailbox *xmbx, u64 *id)
{
	...
	int rc;
	 ...
	 icapxdev = xleaf_get_leaf_by_id(xdev, XRT_SUBDEV_ICAP, XRT_INVALID_DEVICE_INST);
	if (!icapxdev) {
...
static int xmgmt_mailbox_get_mig_calib(struct xmgmt_mailbox *xmbx, u64 *calib)
{
	...
	calibxdev = xleaf_get_leaf_by_id(xdev, XRT_SUBDEV_CALIB, XRT_INVALID_DEVICE_INST);
	if (!calibxdev) {
...
static void xmgmt_mailbox_resp_user_probe(struct xmgmt_mailbox *xmbx, struct xcl_mailbox_req *req,
					  size_t len, u64 msgid, bool sw_ch)
{
	struct xcl_mailbox_conn_resp *resp;
	struct xcl_mailbox_conn *conn;

	resp = vzalloc(sizeof(*resp));
	if (!resp)
		return;

	conn = (struct xcl_mailbox_conn *)req->data;
					   ...
					   size_t len, u64 msgid, bool sw_ch)
{
	...
	kaddr = (struct xcl_mailbox_bitstream_kaddr *)req->data;
	xclbin = (void *)(uintptr_t)kaddr->addr;
	ret = bitstream_axlf_mailbox(xmbx->xdev, xclbin);
...
static ssize_t peer_msg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	xmbx = xdev2mbx(xdev);
	ret = xmgmt_mailbox_get_test_msg(xmbx, false, buf, &len);
	if (ret)
		return ret;
			      ...
			      struct device_attribute *da, const char *buf, size_t count)
{
	...
	xmbx = xdev2mbx(xdev);
	ret = xmgmt_mailbox_set_test_msg(xmbx, (char *)buf, count);
	if (ret)
		return ret;
