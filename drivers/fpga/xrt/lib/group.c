...
static int xrt_grp_probe(struct xrt_device *xdev)
{
	...
	eps = of_find_node_by_name(xdev->dev.of_node, "addressable_endpoints");
	if (!eps) {
		dev_err(&xdev->dev, "failed to get endpoint node");
		return -ENODATA;
	}

	for_each_child_of_node(eps, dn) {
