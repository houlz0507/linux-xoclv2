...
#define XRT_PFW_UNBLOCK_BIT(pf, bar)	BIT((pf) * 6 + (bar))
...
static const struct xrt_device_id xrt_pfw_ids[] = {
	{ XRT_SUBDEV_PCIE_FIREWALL },
	{ }
};
MODULE_DEVICE_TABLE(xrt, xrt_pfw_ids);
...
static struct xrt_driver xrt_pfw_driver = {
	...
	.id_table = xrt_pfw_ids,
	.subdev_id = XRT_SUBDEV_PCIE_FIREWALL,
	.endpoints = xrt_pfw_endpoints,
	.probe = xrt_pfw_probe,
	.leaf_call = xrt_pfw_leaf_call,
};
module_xrt_driver(xrt_pfw_driver);

MODULE_AUTHOR("XRT Team <runtime@xilinx.com>");
MODULE_DESCRIPTION("Xilinx XRT PCIe firewall driver");
